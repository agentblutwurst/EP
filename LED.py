...
try:
    while True:
        ...
        for h in range(len(FrequencyBands)):
            for i in range(10):  # 10 Frequenzbänder
                ...
                end_led = start_led + FrequencyBands[h][i]  # Endposition der Säule (basierend auf dem Wert)
                ...
                for j in range(10):  # Jede Säule hat 10 LEDs
                    ...
                    if led_index < end_led:  # LEDs bis zum Wert des Frequenzbandes leuchten lassen
                        ws.ws2811_led_set(channel, led_index, DOT_COLORS[h])
                    else:
